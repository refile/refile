...
Configure the `Refile.secret_key` using a long, secure string such as one
generated by `SecureRandom.hex(64)`. Example for Rails:

```
$ rake secret # generates secure random string
```

``` ruby
# config/initializers/refile.rb
Refile.configure do |config|
  config.secret_key = '<secure random string>'
end
```
